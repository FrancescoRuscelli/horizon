...
class QRangeSlider(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        diff = self.start_pos - self.getPos(event) # difference between position of mouse when clicked and current position of mouse
        if self.moving == "min":
            ...
            if (temp >= self.bar_width) and (temp < size - self.bar_width): # bar width is the width of the slider
    ...
    def setRange(self, slider_range):
        ...
        self.end = slider_range[1] # seanyeh: I added this to keep track of max
    ...
    def updateDisplayValues(self):
        ...
        self.display_min = int(size * (self.min_val - self.start)/self.scale) + self.bar_width
        self.display_max = int(size * (self.max_val - self.start)/self.scale) + self.bar_width
